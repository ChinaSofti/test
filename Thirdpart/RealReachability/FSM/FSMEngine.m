...
- (id)init
{
    if (self = [super init])
    {
        // created only once
        _allStates = @[
            [ReachStateUnloaded state],
            [ReachStateUnReachable state],
            [ReachStateLoading state],
            [ReachStateWIFI state],
            [ReachStateWWAN state]
        ];
    }
    return self;
}
