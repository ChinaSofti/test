...
{
    return [NSString
    stringWithFormat:@"%c%c %c%c%c%c%c%c%c", (flags & kSCNetworkReachabilityFlagsIsWWAN) ? 'W' : '-',
...
- (void)startNotifier
{
    ...
    if (SCNetworkReachabilitySetCallback (self.reachabilityRef, LocalConnectionCallback, &context))
    {
        ...
        if (!SCNetworkReachabilitySetDispatchQueue (self.reachabilityRef, self.reachabilitySerialQueue))
        {
            ...
            SVError (@"SCNetworkReachabilitySetDispatchQueue() failed: %s", SCErrorString (SCError ()));
        }
    }
    else
    {
        SVError (@"SCNetworkReachabilitySetCallback() failed: %s", SCErrorString (SCError ()));
    }
