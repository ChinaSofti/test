...
#define kDefaultHost @"58.60.106.188"
...
- (void)reachabilityWithBlock:(void (^) (ReachabilityStatus))asyncHandler
{
    ...
    [GPingHelper pingWithBlock:^(BOOL isSuccess) {
      ...
      @{ kEventKeyID: @(RREventPingCallback),
         kEventKeyParam: @(isSuccess) };
...
- (ReachabilityStatus)currentReachabilityStatus
{
    ...
    default:
    {
        SVWarn (@"No normal status matched, return unreachable temporary");
        return RealStatusNotReachable;
    }
    }
}
...
- (NSString *)paramValueFromStatus:(LocalConnectionStatus)status
{
    ...
    default:
    {
        SVError (@"RealReachability error! paramValueFromStatus not matched!");
        return @"";
    }
    }
}
...
- (void)localConnectionChanged:(NSNotification *)notification
{
    ...
    SVInfo (@"currentLocalConnectionStatus:%@", @(lcStatus));

    NSDictionary *inputDic = @{
        kEventKeyID: @(RREventLocalConnectionCallback),
        kEventKeyParam: [self paramValueFromStatus:lcStatus]
    };
