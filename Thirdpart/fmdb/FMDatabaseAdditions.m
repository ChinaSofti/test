...
- (BOOL)tableExists:(NSString *)tableName
{
    ...
    FMResultSet *rs = [self
    executeQuery:@"select [sql] from sqlite_master where [type] = 'table' and lower(name) = ?", tableName];
...
- (FMResultSet *)getSchema
{
    ...
    FMResultSet *rs =
    [self executeQuery:@"SELECT type, name, tbl_name, rootpage, sql FROM (SELECT * FROM "
                       @"sqlite_master UNION ALL SELECT * FROM sqlite_temp_master) WHERE type != "
                       @"'meta' AND name NOT LIKE 'sqlite_%' ORDER BY tbl_name, type DESC, name"];
...
- (BOOL)validateSQL:(NSString *)sql error:(NSError **)error
{
    ...
    while (keepTrying == YES)
    {
        ...
        if (rc == SQLITE_BUSY || rc == SQLITE_LOCKED)
        {
            ...
            {
                SVWarn (@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [self databasePath]);
                SVWarn (@"Database busy");
            }
        }
