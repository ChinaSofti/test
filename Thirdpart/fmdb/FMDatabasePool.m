    ...
    [self executeLocked:^() {
      ...
      if ([_databaseInPool containsObject:db])
      {
          [[NSException exceptionWithName:@"Database already in pool"
                                   reason:@"The FMDatabase being put back into the pool is already "
                                          @"present in the pool"
                                 userInfo:nil] raise];
      }
    ...
    [self executeLocked:^() {
      ...
      else
      {
          ...
          if (_maximumNumberOfDatabasesToCreate)
          {
              ...
              {
                  SVWarn (@"Maximum number of databases (%ld) has already been reached!", (long)currentCount);
                  return;
              }
          }
      ...
      if ([db open])
      {
          if ([_delegate respondsToSelector:@selector (databasePool:shouldAddDatabaseToPool:)] &&
      ...
      {
          SVError (@"Could not open up the database at path %@", _path);
          db = 0x00;
      }
    }];
