...
NSString static *const kYTPlayerCallbackOnYouTubeIframeAPIFailedToLoad =
@"onYouTubeIframeAPIFailedToLoad";

NSString static *const kYTPlayerEmbedUrlRegexPattern = @"^http(s)://(www.)youtube.com/embed/(.*)$";
NSString static *const kYTPlayerAdUrlRegexPattern =
@"^http(s)://pubads.g.doubleclick.net/pagead/conversion/";
NSString static *const kYTPlayerOAuthRegexPattern =
@"^http(s)://accounts.google.com/o/oauth2/(.*)$";
NSString static *const kYTPlayerStaticProxyRegexPattern =
...
- (void)pauseVideo
{
    [self notifyDelegateOfYouTubeCallbackUrl:[NSURL URLWithString:[NSString
                                                                  stringWithFormat:@"ytplayer://"
                                                                                   @"onStateChange"
                                                                                   @"?data=%@",
                                                                                   kYTPlayerStatePausedCode]]];
    ...
    suggestedQuality:(YTPlaybackQuality)suggestedQuality
{
    ...
    NSString *command = [NSString stringWithFormat:@"player.cueVideoById('%@', %@, '%@');", videoId,
                                                   startSecondsValue, qualityValue];
    [self stringFromEvaluatingJavaScript:command];
}
    ...
    suggestedQuality:(YTPlaybackQuality)suggestedQuality
{
    ...
    NSString *qualityValue = [YTPlayerView stringForPlaybackQuality:suggestedQuality];
    NSString *command =
    [NSString stringWithFormat:@"player.cueVideoById({'videoId': '%@', 'startSeconds': %@, "
                               @"'endSeconds': %@, 'suggestedQuality': '%@'});",
     ...
     suggestedQuality:(YTPlaybackQuality)suggestedQuality
{
    ...
    NSString *qualityValue = [YTPlayerView stringForPlaybackQuality:suggestedQuality];
    NSString *command =
    [NSString stringWithFormat:@"player.loadVideoById({'videoId': '%@', 'startSeconds': %@, "
                               @"'endSeconds': %@, 'suggestedQuality': '%@'});",
                               videoId, startSecondsValue, endSecondsValue, qualityValue];
    [self stringFromEvaluatingJavaScript:command];
}
     ...
     suggestedQuality:(YTPlaybackQuality)suggestedQuality
{
    ...
    NSNumber *endSecondsValue = [NSNumber numberWithFloat:endSeconds];
    NSString *qualityValue = [YTPlayerView stringForPlaybackQuality:suggestedQuality];
    NSString *command = [NSString stringWithFormat:@"player.cueVideoByUrl('%@', %@, %@, '%@');", videoURL,
                                                   startSecondsValue, endSecondsValue, qualityValue];
    [self stringFromEvaluatingJavaScript:command];
}
      ...
      suggestedQuality:(YTPlaybackQuality)suggestedQuality
{
    ...
    NSNumber *endSecondsValue = [NSNumber numberWithFloat:endSeconds];
    NSString *qualityValue = [YTPlayerView stringForPlaybackQuality:suggestedQuality];
    NSString *command = [NSString stringWithFormat:@"player.loadVideoByUrl('%@', %@, %@, '%@');", videoURL,
                                                   startSecondsValue, endSecondsValue, qualityValue];
    [self stringFromEvaluatingJavaScript:command];
}
...
- (BOOL)loadWithPlayerParams:(NSDictionary *)additionalPlayerParams
{
    ...
    if (jsonRenderingError)
    {
        NSLog (@"Attempted configuration of player with invalid playerVars: %@ \tError: %@",
               playerParams, jsonRenderingError);
        return NO;
    }
   ...
   suggestedQuality:(YTPlaybackQuality)suggestedQuality
{
    ...
    NSNumber *startSecondsValue = [NSNumber numberWithFloat:startSeconds];
    NSString *qualityValue = [YTPlayerView stringForPlaybackQuality:suggestedQuality];
    NSString *command = [NSString stringWithFormat:@"player.cuePlaylist(%@, %@, %@, '%@');", cueingString,
                                                   indexValue, startSecondsValue, qualityValue];
    [self stringFromEvaluatingJavaScript:command];
}
    ...
    suggestedQuality:(YTPlaybackQuality)suggestedQuality
{
    ...
    NSNumber *startSecondsValue = [NSNumber numberWithFloat:startSeconds];
    NSString *qualityValue = [YTPlayerView stringForPlaybackQuality:suggestedQuality];
    NSString *command = [NSString stringWithFormat:@"player.loadPlaylist(%@, %@, %@, '%@');", cueingString,
                                                   indexValue, startSecondsValue, qualityValue];
    [self stringFromEvaluatingJavaScript:command];
}
...
+ (NSBundle *)frameworkBundle
{
    ...
    dispatch_once (&predicate, ^{
      ...
      [mainBundlePath stringByAppendingPathComponent:@"Assets.bundle"];
