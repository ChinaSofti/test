...
- (UIImageView *)imgViewType {
  if (_imgViewType == nil) {
    ...
    _imgViewType.frame = CGRectMake(Gap * 3.3, (CellHeight - LabelHeight) / 2,
                                    kScreenW / 15, kScreenH / 25);
  }
  return _imgViewType;
}
//时间——月日
- (UILabel *)testDate {
  if (_testDate == nil) {
    ...
    _testDate.frame = CGRectMake(kScreenW / 5 - 5,
                                 (CellHeight - LabelHeight - TimeHeight) / 2,
...
- (UILabel *)videoMOS {
  if (_videoMOS == nil) {
    ...
    _videoMOS.frame =
        CGRectMake(kScreenW * 2 / 5 - 5, (CellHeight - LabelHeight) / 2,
                   kScreenW / 5, LabelHeight);
...
- (UILabel *)loadTime {
  if (_loadTime == nil) {
    ...
    _loadTime.frame =
        CGRectMake(kScreenW * 3 / 5 - 5, (CellHeight - LabelHeight) / 2,
                   kScreenW / 5, LabelHeight);
...
- (UILabel *)bandWidth {
  if (_bandWidth == nil) {
    ...
    _bandWidth.frame =
        CGRectMake(kScreenW * 4 / 5 - 5, (CellHeight - LabelHeight) / 2,
                   kScreenW / 6, LabelHeight);
...
- (void)setResultModel:(SVSummaryResultModel *)resultModel {
  ...
  NSDate *date = [[NSDate alloc]
      initWithTimeIntervalSince1970:([testTime longLongValue] / 1000)];
  ...
  self.videoMOS.text =
      [NSString stringWithFormat:@"%.2fms", [resultModel.UvMOS floatValue]];
  self.loadTime.text =
      [NSString stringWithFormat:@"%@ms", resultModel.loadTime];
  self.bandWidth.text =
      [NSString stringWithFormat:@"%@kbps", resultModel.bandwidth];
}
