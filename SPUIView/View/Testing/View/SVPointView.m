...
- (instancetype)init {
  if ([super init]) {
    ...
    //设置Label
    _uvMosBarView =
        [[UIView alloc] initWithFrame:CGRectMake(FITWIDTH(20), FITWIDTH(100),
                                                 FITWIDTH(80), FITWIDTH(20))];

    _speedLabel = [CTWBViewTools
        createLabelWithFrame:CGRectMake(_uvMosBarView.rightX + FITWIDTH(25),
                                        FITWIDTH(100), FITWIDTH(80),
                                        FITWIDTH(20))
                    withFont:16
    ...
    _bufferLabel = [CTWBViewTools
        createLabelWithFrame:CGRectMake(_speedLabel.rightX + FITWIDTH(35),
                                        FITWIDTH(100), FITWIDTH(50),
                                        FITWIDTH(20))
                    withFont:16
              withTitleColor:RGBACOLOR(250, 180, 86, 1)
                   withTitle:@"0"];
    ...
    _uvMosNumLabel = [CTWBViewTools
        createLabelWithFrame:CGRectMake(FITWIDTH(15),
                                        _uvMosBarView.bottomY + FITWIDTH(10),
    ...
    // 2.TestingView中的初始化
    _panelView =
        [[UIView alloc] initWithFrame:CGRectMake(FITWIDTH(20), FITWIDTH(160),
                                                 FITWIDTH(280), FITWIDTH(280))];
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectZero];
    imageView.size = CGSizeMake(280, 280);
    imageView.image = [UIImage imageNamed:@"clock_video_panel"];
    imageView.center = CGPointMake(_panelView.frame.size.width / 2,
    ...
    imageView2.center = CGPointMake(_panelView.frame.size.width / 2,
                                    _panelView.frame.size.height / 2);
    [_middleView addSubview:imageView2];

    _grayView = [[SVPointView alloc]
        initWithFrame:CGRectMake(FITWIDTH(20), FITWIDTH(160), FITWIDTH(280),
                                 FITWIDTH(280))];
    ...
    imageView3.center = CGPointMake(_panelView.frame.size.width / 2,
                                    _panelView.frame.size.height / 2);
    [_grayView addSubview:imageView3];

    _label1 =
        [[UILabel alloc] initWithFrame:CGRectMake(FITWIDTH(130), FITWIDTH(290),
    ...
    _label2 =
        [[UILabel alloc] initWithFrame:CGRectMake(FITWIDTH(110), FITWIDTH(350),
                                                  FITWIDTH(100), FITWIDTH(50))];
    _label2.text = @"0.00";
    ...
    _pointView = [[[NSBundle mainBundle] loadNibNamed:@"SVPointView"
                                                owner:nil
    ...
    _placeLabel = [CTWBViewTools
        createLabelWithFrame:CGRectMake(FITWIDTH(170), FITWIDTH(425),
                                        FITWIDTH(150), FITWIDTH(20))
                    withFont:16
              withTitleColor:[UIColor blackColor]
                   withTitle:@""];

    _resolutionLabel = [CTWBViewTools
        createLabelWithFrame:CGRectMake(FITWIDTH(230), FITWIDTH(470),
                                        FITWIDTH(80), FITWIDTH(20))
                    withFont:10
              withTitleColor:[UIColor blackColor]
                   withTitle:@""];
    ...
    _bitLabel = [CTWBViewTools
        createLabelWithFrame:CGRectMake(FITWIDTH(230), FITWIDTH(495),
                                        FITWIDTH(80), FITWIDTH(20))
                    withFont:10
              withTitleColor:[UIColor blackColor]
                   withTitle:@""];
    ...
    _placeNumLabel = [CTWBViewTools
        createLabelWithFrame:CGRectMake(FITWIDTH(170), FITWIDTH(445),
                                        FITWIDTH(150), FITWIDTH(20))
                    withFont:12
    ...
    _resolutionNumLabel = [CTWBViewTools
        createLabelWithFrame:CGRectMake(FITWIDTH(170), FITWIDTH(470),
                                        FITWIDTH(50), FITWIDTH(20))
                    withFont:10
              withTitleColor:[UIColor lightGrayColor]
                   withTitle:@"分辨率"];

    _bitNumLabel = [CTWBViewTools
        createLabelWithFrame:CGRectMake(FITWIDTH(170), FITWIDTH(495),
                                        FITWIDTH(50), FITWIDTH(20))
                    withFont:10
              withTitleColor:[UIColor lightGrayColor]
                   withTitle:@"码率"];
...
- (void)start {
  ...
  CADisplayLink *link =
      [CADisplayLink displayLinkWithTarget:self selector:@selector(rotate)];
...
- (void)updateUvMOS:(float)uvMOS {
  ...
  if (uvMOS != _num) {
    ...
    if (uvMOS >= 2.5) {
      self.grayView.transform =
          CGAffineTransformMakeRotation(uvMOS - 2.5 / 1.2);
    }
    _num = uvMOS;
...
//        [self.label2SuperView insertSubview:_label2
//        atIndex:self.label2IndexInSuperView];
//    }
//    if (self.num > M_PI * 2 / 3)
//    {
