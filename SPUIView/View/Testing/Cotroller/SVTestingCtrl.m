...
- (void)viewDidLoad {
  ...
  UIImageView *imageView = [[UIImageView alloc]
      initWithFrame:CGRectMake(FITWIDTH(0), FITWIDTH(0), FITWIDTH(100),
                               FITWIDTH(30))];
  //设置图片名称
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  _videoTest = [[SVVideoTest alloc] initWithView:testId
...
- (void)creatVideoView {
  ...
  _videoView = [[SVPointView alloc]
      initWithFrame:CGRectMake(FITWIDTH(10), FITWIDTH(420), FITWIDTH(150),
                               FITWIDTH(92))];
  ...
  //添加视频点击事件
  UIButton *bgBtn =
      [[UIButton alloc] initWithFrame:CGRectMake(FITWIDTH(10), FITWIDTH(420),
                                                 FITWIDTH(150), FITWIDTH(92))];
...
- (void)updateTestResultDelegate:(SVVideoTestContext *)testContext
  ...
  NSLog(@"uvMOSSession: %f  firstBufferTime:%ld  cuttonTimes:%d  location:%@  "
        @"bitrate:%f  "
        @"videoResolution:%@",
        uvMOSSession, firstBufferTime, cuttonTimes, location, bitrate,
        videoResolution);
  dispatch_async(dispatch_get_main_queue(), ^{
    [_footerView.placeLabel setText:location];
    [_footerView.resolutionLabel setText:videoResolution];
    [_footerView.bitLabel
        setText:[NSString stringWithFormat:@"%.2fkpbs", bitrate]];
    [_headerView.bufferLabel
        setText:[NSString stringWithFormat:@"%d", cuttonTimes]];
    [_headerView.speedLabel
        setText:[NSString stringWithFormat:@"%ldms", firstBufferTime]];
    ...
    //      [_headerView.uvMosLabel setText:[NSString stringWithFormat:@"%.2f",
    //      uvMOSSession]];
...
- (void)changeValueUseFakeData {
  ...
  float fakeBitrate = [[NSString
      stringWithFormat:@"%d.%d", firstFakeBitrate, lastFakeBitrate] floatValue];
  //    NSLog (@"fake bitrate:%f", fakeBitrate);
  [_footerView.bitLabel
      setText:[NSString stringWithFormat:@"%.2fkpbs", fakeBitrate]];

  int firstFakeUvMOSSession = (int)realuvMOSSession;
  int lastFakeUvMOSSession = arc4random() % 50;
  float fakeUvMOSSession =
      [[NSString stringWithFormat:@"%d.%d", firstFakeUvMOSSession,
                                  lastFakeUvMOSSession] floatValue];
  ...
  if (_resultTimes % 10 == 0) {
    ...
    if (_UvMOSbarResultTimes < 20) {
      ...
      UUBar *bar = [[UUBar alloc]
          initWithFrame:CGRectMake(10 + _UvMOSbarResultTimes * 3, 0, 1, 30)];
