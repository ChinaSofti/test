...
{
    UITextField * textField = [self createTextFieldWithFrame:frame placeholder:placeholder passWord:YESorNO leftView:leftView rightView:rightView Font:font fontColor:[UIColor blackColor]];
    ...
    if (lineColor) {
        ...
        UIView * lineView = [self lineViewWithFrame:CGRectMake(frame.origin.x, frame.size.height - 1, width, 0.5) withColor:lineColor];
...
+ (UIView *)viewWithFrame:(CGRect)frame withImage:(UIImage *)image withTitle:(NSString *)title
{
    ...
    if (title) {
        ...
        UILabel * label = [[UILabel alloc] initWithFrame:CGRectMake(imageV.rightX + 5, imageV.originY + 5, view.width - imageV.rightX - 5, 20)];
...
+ (UIButton *)createBtnWithFrame:(CGRect)frame withImage:(NSString *)btnImage withImgFrame:(CGRect)imgFrame withTitle:(NSString *)title withLabelFrame:(CGRect)labelFrame
{
    ...
    for (UIView * view in button.subviews) {
        ...
        if ([view isKindOfClass:[UILabel class]]) {
            ...
            {
                view.frame = CGRectMake(0, imgFrame.size.height, frame.size.width, frame.size.height - imgFrame.size.height);
            }
        }
    }
...
//qq和微信登陆按钮
+ (UIButton *)createBtnWithFrame:(CGRect)frame withImage:(NSString *)btnImage withTitle:(NSString *)title
{
    UIButton * button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.frame = frame;
    
    UIImageView * topImageV = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, button.width, button.width)];
    topImageV.image = [UIImage imageNamed:btnImage];
    
    [button addSubview:topImageV];
    
    UILabel * titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(-FITWIDTH(10), topImageV.bottomY + FITWIDTH(10), button.width + FITWIDTH(20), button.height - topImageV.bottomY - FITWIDTH(3))];
...
+ (UIButton *)createBtnWithFrame:(CGRect)frame backgroundImage:(NSString *)backgroundImage hightlightedImage:(NSString *)hightlighted title:(NSString *)title titleColor:(UIColor *)titleColor
{
    ...
    [button setBackgroundImage:[UIImage imageNamed:hightlighted] forState:UIControlStateHighlighted];
...
{
    UIButton * button = [self createBtnWithFrame:frame normalImage:normalImage highlighImage:highlighImage seleceImage:selectImage normalTitle:normalTitle normalColor:normalColor];
...
{
    CGSize size = [string boundingRectWithSize:bigSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:font]} context:nil].size;
...
+ (UIAlertController *)alertViewWithTitle:(NSString *)title message:(NSString *)message okTitle:(NSString *)okTitle okClick:(void(^)(void))okClick  cancelTitle:(NSString *)cancelTitle  cancelClick:(void(^)(void))cancelClick
{
    ...
    [alertView addAction:[UIAlertAction actionWithTitle:okTitle style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
        okClick();
    }]];
    
    [alertView addAction:[UIAlertAction actionWithTitle:cancelTitle style:UIAlertActionStyleCancel handler:^(UIAlertAction * action) {
        cancelClick();
    }]];
...
+ (BOOL)isContainsEmoji:(NSString *)string {
    ...
    [string enumerateSubstringsInRange:NSMakeRange(0, [string length]) options:NSStringEnumerationByComposedCharacterSequences usingBlock:     ^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {         const unichar hs = [substring characterAtIndex:0];
        if (0xd800 <= hs && hs <= 0xdbff) {
            if (substring.length > 1) {
                const unichar ls = [substring characterAtIndex:1];                 const int uc = ((hs - 0xd800) * 0x400) + (ls - 0xdc00) + 0x10000;
