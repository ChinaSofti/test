...
- (BOOL)application:(UIApplication *)application
  ...
  UIImageView *splashImageView = [[UIImageView alloc]
      initWithImage:[UIImage imageNamed:@"starting_window"]];
...
- (void)loadResouceFromServer {
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),
                 ^{
                   // 初始化Test Context
                   SVTestContextGetter *contextGetter =
                       [SVTestContextGetter sharedInstance];
                   // 初始化本机IP和运营商等信息
